/* File:     floyd.c
 * Purpose:  Implement the Floyd-Warshall algorithm for solving the 
 *           all-pairs shortest path problem:  find the length of the 
 *           shortest path between each pair of vertices in a directed 
 *           graph.
 *
 ...
 *     column is mat[i*n + j]
 * 7.  Use the compile flag -DSHOW_INT_MATS to print the matrix after it's
...
int main(void) {
   ...
   //Print_matrix(mat, n);
   printf("Sequential ellapsed time: %lf \n", time_spent);
